...
def convert_mtx_to_gr_dir_weighted(mtx_file: Path):
    ...
    # gr -> tgr
    run([GRAPH_CONVERT, "--gr2tgr", str(gr_file), str(tgr_file)])
...
def convert_mtx_to_gr_undir_weighted(mtx_file: Path):
    ...
    run([GRAPH_CONVERT, "--gr2sgr", str(gr_file), str(gr_file), "--edgeType=float32"])
...
def convert_mtx_to_gr_undir(mtx_file: Path):
    ...
    # mtx -> gr
    run([GRAPH_CONVERT, "--edgelist2gr", str(mtx_file), str(gr_file)])

    # gr -> gr + reversed edges
    run([GRAPH_CONVERT, "--gr2sgr", str(gr_file), str(gr_file)])
